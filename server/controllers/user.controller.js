...
const addUser = async (req, res, next) => {
  try {
    ...
    // Cloudinary returns: req.file.path
    const avatarUrl = req.file?.path || defaultAvatar;

    const emailDomain = req.body.email.split("@")[1];
    const role = emailDomain === "mod.vibeshare.com" ? "moderator" : "general";
    ...
    // go to context-based auth setup
